...
function getTeammember(projectId: string, teammemberId: number): any {
  ...
  try {
    const teammember = db
      .query("select * from teammembers where id = ?")
      .get(teammemberId);
    if (!teammember) {
...
function deleteTeammember(projectId: string, teammemberId: number): void {
  ...
  try {
    const tasks = db
      .query("select * from tasks where teammember_id = ?")
      .all(teammemberId);
    if (tasks.length > 0) {
      throw new Error(
        "No se puede eliminar el teammember porque tiene tareas asignadas"
      );
    }
...
teammembers.post("/", async (c) => {
  try {
    ...
    if (!projectExists(projectIdStr)) {
      return c.json(
        { error: `Proyecto con id ${project_id} no encontrado` },
        404
      );
    }
    ...
    return c.json(
      {
        error: "Error al crear el teammember",
...
teammembers.put("/", async (c) => {
  try {
    ...
    return c.json(
      {
        error: "Error al actualizar el teammember",
        details: error instanceof Error ? error.message : String(error),
      },
      500
    );
...
teammembers.get("/:id", async (c) => {
  try {
    ...
    return c.json(
      {
        error: "Error al obtener el teammember",
        details: error instanceof Error ? error.message : String(error),
      },
      500
    );
...
teammembers.delete("/", async (c) => {
  try {
    ...
    return c.json({
      message: "Teammember eliminado correctamente",
      id: teammemberId,
    });
  } catch (error) {
    return c.json(
      {
        error: "Error al eliminar el teammember",
        details: error instanceof Error ? error.message : String(error),
      },
      500
    );
