...
function createTeam(projectId: string, teamData: any): any {
  ...
  try {
    ...
    const result = insertQuery.run(teamData.name, teamData.description);
...
function deleteTeam(projectId: string, teamId: number): void {
  ...
  try {
    ...
    const members = db
      .query("select * from teammembers where team_id = ?")
      .all(teamId);
    if (members.length > 0) {
      throw new Error(
        "No se puede eliminar el team porque tiene miembros asignados"
      );
    }
